...
def create_app():
    ...
    model = pickle.load(open('carpricen.sav', 'rb'))

    # Load car data from CSV
    car_data = pd.read_csv('data/unique_models.csv')
    ...
    def predict():
        if request.method == 'POST':
            ...
            transmission = 'Manual'  # Ensure this matches your form input
            ...
            result = model.predict([[encoded_name, year, km_driven, encoded_fuel, encoded_seller_type, encoded_transmission, encoded_owner, mileage, engine, seats]])
...
# Entry point when running with 'flask run' or 'waitress-serve'
app = create_app()
